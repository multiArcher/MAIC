...
class RewardShapingLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.potential_scale = args.potential_sum / args.env_info["episode_limit"]
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        rewards, F_potential = self._potential_reward(batch)
        # modified rewards = r + F
        shaped_rewards = rewards + F_potential
        # TODO
        ...
        if self.args.standardise_rewards:
            self.rew_ms.update(shaped_rewards)
            shaped_rewards = (shaped_rewards - self.rew_ms.mean) / th.sqrt(self.rew_ms.var)

        if self.args.common_reward:
            assert (
                shaped_rewards.size(2) == 1
            ), "Expected singular agent dimension for common rewards"
            # reshape rewards to be of shape (batch_size, episode_length, n_agents)
            shaped_rewards = shaped_rewards.expand(-1, -1, self.n_agents)
        ...
        targets = (
            shaped_rewards + self.args.gamma * (1 - terminated) * target_max_qvals.detach()
        )
    ...
    def _potential_reward(self, batch: EpisodeBatch) -> tuple[th.Tensor, th.Tensor]:
        # calculate reward and potential
        rewards = batch["reward"][:, :-1]
        states = batch["state"]

        potential = self._calculate_potential(states, batch.max_seq_length)
        F = potential[:, 1:] - potential[:, :-1]

        return rewards, F

    def _calculate_potential(self, states: th.Tensor, max_t: int) -> th.Tensor:
        # return potential of every state
        # time_cost = th.linspace(
        #     0,
        #     self.args.potential_per_agent * self.n_agents,
        #     steps=self.args.env_info["episode_limit"] + 1,
        #     device=self.args.device
        # )[:max_t]

        time_cost = th.ones(max_t, device=self.args.device) * self.potential_scale
        
        return -time_cost.reshape(1, -1, 1).repeat(states.shape[0], 1, 1)
