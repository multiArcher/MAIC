...
class EntityStateAsObsMAC(MAC):
    ...
    def __init__(self, scheme: dict, groups, args: SimpleNamespace):
        ...
        super(EntityStateAsObsMAC, self).__init__(scheme, groups, args)
        ...
        self.state_components: dict[str: tuple[int, int]] = args.env_info["state_components"]
    ...
    def forward(self, ep_batch, t, test_mode=False, *args, **kwargs):
        ...
        # TODO: For politic action selection. Not Implemented yet.
    ...
    """
        # Input from env.
        input_scheme = self.state_components
    ...
    def _build_inputs(self, batch, t: slice):
        ...
        """
        state_data = batch["state"][:, t].unsqueeze(-2).repeat(1, 1, self.n_agents, 1)  # expand state to individual obs.
        batch_size, max_length, n_agents, state_size = state_data.shape
        time_size = state_data.shape[1]
        inputs = []

        # Split obs by the mapping indices.
        split_state = torch.split(state_data, self.input_splits, dim=-1)
        for i, feat_shape in enumerate(self.input_scheme[0].values()):
            inputs.append(
                split_state[i].reshape(batch_size, time_size, n_agents, *feat_shape)
            )
