...
class BCRBCMAC(MAC):
    ...
    def __init__(self, scheme: dict, groups: dict, args: SimpleNamespace):
        ...
        self.agent = BCRBCDynamicsCore(self.input_shape, self.n_actions, self.n_agents, args, recon_obs_dim=self.obs_shape)
        self.use_comm = getattr(args, "bcrbc_use_comm", False)
        if self.use_comm:
            self.comm_delay = CommDelay(
                self.n_agents,
                delay_mean=getattr(args, "comm_gaussian_delay_mean", 1.0),
                delay_std=getattr(args, "comm_gaussian_delay_std", 1.0),
        ...
        self.use_generative_eval = getattr(args, "bcrbc_generative_eval", False)
    ...
    def forward(self, ep_batch: EpisodeBatch, t: int | slice, test_mode=False, **kwargs):
        ...
        if (
            ...
            and all(k in ep_batch.scheme for k in ("obs_gen_t", "obs_fresh_mask"))
    ...
    def _rollout_latent_buffer(self, ep_batch: EpisodeBatch, end: int):
        ...
        is_real = torch.zeros(B, end, n, device=device)
        local_gen = gen_t.clamp(min=0, max=end - 1)
        idx = local_gen.view(B, end, n, 1, 1).expand(-1, -1, -1, nt, zd)
        z_buf.scatter_(1, idx, z_delivered)
        is_real.scatter_(1, local_gen, torch.ones(B, end, n, device=device))

        clean_meta = self._clean_window_metadata(B, end, n, device)
        ...
        for tau in range(end):
            ...
            gen_z = self.agent.flow_dynamics.sample(
                belief_prev, steps=getattr(self.args, "bcrbc_flow_steps", 8)
            )  # [B, n, num_token, z_dim]
            real_mask = is_real[:, tau].view(B, n, 1, 1)
    ...
    def belief_diagnostics(self, ep_batch: EpisodeBatch, full_obs: torch.Tensor) -> dict:
        ...
        obs_aug_full, last_actions = self._build_inputs(ep_batch, slice(0, T), obs_override=full_obs.to(device))
        obs_aug_full = obs_aug_full.to(device)
        last_actions = last_actions.to(device)
        z_full = self.agent.latent_encoder(obs_aug_full).reshape(B, T, n, nt, zd)

        z_corr, is_real, _, _, clean_meta = self._rollout_latent_buffer(ep_batch, T)

        # masks carry the token axis (broadcast over it): [B, T, n, 1, 1].
        gen_mask = (is_real < 1).view(B, T, n, 1, 1).float()
        ...
        out = self.agent(obs_aug_full, last_actions, start_t=0,
                         z_override=z_corr, **clean_meta[slice(0, T)])
        beliefs = out["beliefs"]
        gen_est = z_corr.clone()
        if T > 1:
            gen_est[:, 1:] = self.agent.flow_dynamics.sample(
                beliefs[:, :-1], steps=getattr(self.args, "bcrbc_flow_steps", 8)
            )
        delay = ep_batch["obs_delay"][:, :T].to(device).squeeze(-1)
        delayed_real = ((is_real > 0).float() * (delay > 0).float()).view(B, T, n, 1, 1)
    ...
    def _clean_window_metadata(self, B: int, length: int, n: int, device):
        ...
        """
        gen = torch.arange(length, device=device).view(1, length, 1, 1).expand(B, -1, n, -1).long()

        class _Meta:
            def __getitem__(self, sl):
                return {
                    "obs_delay": torch.zeros(B, sl.stop - (sl.start or 0), n, 1, dtype=torch.long, device=device),
                    "obs_gen_t": gen[:, sl],
                    "obs_fresh_mask": torch.ones(B, sl.stop - (sl.start or 0), n, 1, device=device),
                }
    ...
    def _build_agents(self, input_shape):
        self.agent = BCRBCDynamicsCore(input_shape, self.n_actions, self.n_agents, self.args, recon_obs_dim=self.obs_shape)

    def _build_delay_metadata(self, batch, t: slice):
        if not all(k in batch.scheme for k in ("obs_delay", "obs_gen_t", "obs_fresh_mask")):
            return {}
        return {
