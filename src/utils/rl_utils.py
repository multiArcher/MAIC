...
def new_build_td_lambda_targets(rewards, terminated, mask, target_qs, gamma, td_lambda):
    # Assumes  <target_qs > in B*(T-1)*A and <reward >, <terminated >, <mask > in (at least) B*T-1*1

    target_qs = target_qs.reshape(*target_qs.shape[:3])
...
class RunningMeanStd(object):
    ...
    def update_from_moments(self, batch_mean, batch_var, batch_count):
        ...
        M2 = m_a + m_b + numpy.square(delta) * self.count * batch_count / (self.count + batch_count)
