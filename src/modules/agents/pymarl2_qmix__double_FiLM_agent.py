...
class FiLMAgent(nn.Module):
    def __init__(self, input_shape, args: SimpleNamespace):
        ...
        self.FiLM_layer1 = FiLMLayer(self.unit_types, args.rnn_hidden_dim)
        self.FiLM_layer2 = FiLMLayer(self.unit_types, args.n_actions)
    ...
    def forward(self, inputs, hidden_state):
        ...
        x = self.FiLM_layer1(x, torch.argmax(inputs[:, self.unit_type_slice], dim=1).detach())
        h_in = hidden_state.reshape(-1, self.args.rnn_hidden_dim)
        hh = self.rnn(x, h_in)

        q = self.q_projection(hh)

        q = self.FiLM_layer2(q, torch.argmax(inputs[:, self.unit_type_slice], dim=1).detach())
